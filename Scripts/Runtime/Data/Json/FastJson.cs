...
namespace MPGameLib.Data
{
	public static class FastJson
	{
		...
		static Serializer _GetSerializer(System.Type type)
		{
			...
			MethodInfo method = type.GetMethod("ToJson",
				BindingFlags.Static | BindingFlags.NonPublic | BindingFlags.Public | BindingFlags.IgnoreCase, null,
				serializerrArgs, null);
		...
		public class JsonParser
		{
			...
			public void ReadSpecifyChar(char spec)
			{
				...
				throw new System.FormatException("failed read char '" + spec + "'. index = " + count + " text = \"" +
				                                 text + "\"");
			}
		...
		static List<object> _ParseArray(JsonParser parser, System.Type elementType, Deserializer deserializer, int nest,
			ref int[] length)
		{
			parser.ReadSpecifyChar('[');
	...
	{
		HashSet<object> 	dummy1 =
 new HashSet<object> ( new DeserializeEnumerator<object>(null,null) );		dummy1.Add (null);
